...
impl OpenAIRequestHelper {
    pub fn new(client: reqwest::Client, base_url: &str, api_key: &str) -> Self {
        let config =
            RequestConfig::new(base_url).with_auth(AuthMethod::Bearer(api_key.to_string()));
    ...
    pub async fn send_chat_request(
        &self,
        request: &ChatRequest,
    ) -> Result<ChatResponse, OpenAIRequestError> {
    ...
    pub async fn cancel_fine_tuning_job(
        ...
        let endpoint = Endpoint::new(
            format!("fine_tuning/jobs/{}/cancel", job_id),
            HttpMethod::Post,
        );
        Ok(self
            .request_builder
    ...
    pub async fn upload_file(
        ...
        Ok(self
            .request_builder
            .request_multipart(&endpoint, form.build())
            .await?)
    }
    ...
    pub async fn send_responses_request(
        ...
        let mut response: ResponsesResponse = self
            .request_builder
            .request_json(&endpoint, Some(request))
            .await?;
