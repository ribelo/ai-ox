use openai_ox::{OpenAI, ReasoningConfig, ResponsesInput};
use std::env;
    ...
    if has_encrypted {
        ...
        for reasoning in reasoning_items {
            if !reasoning.1.is_empty() {
                println!(
                    "✓ Found encrypted content for reasoning item: {}",
                    reasoning.0
                );
            }
        }
...
async fn test_real_responses_api_streaming() {
    ...
    if let Some(usage) = final_usage {
        println!(
            "Final usage - Input: {}, Output: {}, Total: {}",
            usage.input_tokens, usage.output_tokens, usage.total_tokens
        );
        if let Some(reasoning_tokens) = usage.reasoning_tokens {
            println!("Reasoning tokens: {}", reasoning_tokens);
...
async fn test_real_responses_api_with_messages() {
    ...
    assert!(
        content.to_lowercase().contains("15")
            || content.contains("eight")
            || content.contains("seven")
    );
}
