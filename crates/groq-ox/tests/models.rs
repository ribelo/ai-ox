...
async fn test_list_models() {
    ...
    let _has_audio_model = response.data.iter().any(|m| m.is_speech_to_text() || m.is_text_to_speech());
    ...
    assert!(!first_model.owned_by.is_empty(), "Model owner should not be empty");
    assert!(first_model.context_window > 0, "Context window should be positive");
...
async fn test_get_specific_model() {
    ...
    let model = client.get_model(&first_model.id).await
