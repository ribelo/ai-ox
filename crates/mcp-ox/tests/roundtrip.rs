...
fn test_basic_tool_call_roundtrip() {
    ...
    assert_eq!(
        args.get("x_ai_ox_tool_call_id").unwrap().as_str().unwrap(),
        "call_123"
    );
...
fn test_strict_vs_lenient_modes() {
    ...
    let result_lenient =
        Part::from_mcp_with_config(mcp_response_no_meta.clone(), &lenient_config).unwrap();
    ...
    let result_incomplete_lenient =
        Part::from_mcp_with_config(mcp_response_incomplete_meta.clone(), &lenient_config).unwrap();
    ...
    let result_incomplete_strict =
        Part::from_mcp_with_config(mcp_response_incomplete_meta, &strict_config);
