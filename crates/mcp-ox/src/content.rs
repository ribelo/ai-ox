use ai_ox::content::{Message, Part, DataRef};
use mcp_sdk::types::ToolResponseContent;
use url::Url;
use crate::{McpConversionError, ToMcp, FromMcp};
        ...
        Part::Blob { data_ref, mime_type, .. } => match data_ref {
            DataRef::Base64 { data } => {
                if mime_type.starts_with("image/") {
                    Ok(ToolResponseContent::Image { data: data.clone(), mime_type: mime_type.clone() })
                } else {
                    Err(McpConversionError::InvalidFormat("Base64 blobs not supported for ToolResponseContent unless image".to_string()))
                }
            }
            DataRef::Uri { uri } => {
                Ok(ToolResponseContent::Resource {
                    resource: mcp_sdk::types::ResourceContents {
                        uri: Url::parse(uri).map_err(|_| McpConversionError::InvalidFormat("Invalid URI".to_string()))?,
                        mime_type: Some(mime_type.clone()),
                    }
                })
            }
        },
        _ => Err(McpConversionError::InvalidFormat("Unsupported Part type for ToolResponseContent".to_string())),
    }
}

fn tool_response_content_to_part(content: &ToolResponseContent) -> Result<Part, McpConversionError> {
    match content {
        ToolResponseContent::Text { text } => Ok(Part::Text { text: text.clone(), ext: std::collections::BTreeMap::new() }),
        ...
        ToolResponseContent::Resource { resource } => Ok(Part::Blob {
            data_ref: DataRef::Uri { uri: resource.uri.to_string() },
