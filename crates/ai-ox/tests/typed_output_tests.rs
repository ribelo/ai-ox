...
async fn test_all_providers_simple_typed_output() {
    ...
    for model in models {
        ...
        let agent = Agent::builder()
            .model(model.into())
            .system_instruction(
                "You are a helpful assistant that returns JSON responses exactly as requested.",
            )
            .build();
        ...
        let messages = vec![Message::new(
            MessageRole::User,
            vec![Part::Text {
                text: "Return a simple JSON object with a message 'hello world' and number 42"
                    .to_string(),
                ext: BTreeMap::new(),
            }],
        )];
        ...
        match result {
            Ok(response) => {
                ...
                println!(
                    "✅ Model {} passed simple typed output test",
                    response.model_name
                );
            }
            Err(e) => {
...
async fn test_all_providers_complex_typed_output() {
    ...
    for model in models {
        let model_name = model.name().to_string();
        println!(
            "\n--- Testing complex typed output with model: {} ---",
            &model_name
        );
            ...
            Ok(response) => {
                ...
                for (i, item) in response.data.items.iter().enumerate() {
                    ...
                    assert!(
                        ...
                        "Item {} should have a valid value",
                        i
                    );
                }

                println!(
                    "✅ Model {} passed complex typed output test",
                    response.model_name
                );
            }
            Err(e) => {
                ...
                println!(
                    "⚠️  Model {} does not fully support complex typed output",
                    &model_name
                );
            }
        }
...
async fn test_all_providers_invalid_json_parsing() {
    ...
    for model in models {
        let model_name = model.name().to_string();
        println!(
            "\n--- Testing invalid JSON handling with model: {} ---",
            &model_name
        );
        ...
        let agent = Agent::builder()
            .model(model.into())
            .system_instruction(
                "Return the text 'this is not json' exactly as given, with no JSON formatting.",
            )
            .build();
        ...
        match result {
            Ok(_) => {
                println!(
                    "⚠️  Model {} unexpectedly returned valid JSON when asked not to",
                    &model_name
                );
            ...
            Err(e) => {
                ...
                assert!(
                    error_str.contains("Failed to parse response")
                ...
                println!(
                    "✅ Model {} correctly errored on non-JSON response",
                    &model_name
                );
            }
        }
