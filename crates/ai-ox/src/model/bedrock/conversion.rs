...
fn json_to_document(value: Value) -> aws_smithy_types::Document {
    match value {
        ...
        Value::Array(arr) => aws_smithy_types::Document::Array(
            arr.into_iter().map(json_to_document).collect()
        ),
        Value::Object(obj) => aws_smithy_types::Document::Object(
            obj.into_iter().map(|(k, v)| (k, json_to_document(v))).collect()
        ),
    }
}
...
pub(super) fn document_to_json(doc: &aws_smithy_types::Document) -> Value {
    match doc {
        ...
        aws_smithy_types::Document::Number(n) => match n {
            ...
            aws_smithy_types::Number::Float(f) => Value::Number(serde_json::Number::from_f64(*f).unwrap_or_else(|| 0.into())),
        },
        aws_smithy_types::Document::String(s) => Value::String(s.clone()),
        aws_smithy_types::Document::Array(arr) => Value::Array(
            arr.iter().map(document_to_json).collect()
        ),
        aws_smithy_types::Document::Object(obj) => Value::Object(
            obj.iter().map(|(k, v)| (k.clone(), document_to_json(v))).collect()
        ),
    }
}
...
pub(super) fn convert_ai_ox_messages_to_bedrock(
    ...
    for message in messages {
        ...
        let bedrock_message = BedrockMessage::builder()
            ...
            .build()
            .map_err(|e| BedrockError::MessageConversion(format!("Failed to build message: {}", e)))?;
        ...
        let content_block = match part {
            ...
            Part::ToolUse { id, name, args, .. } => {
                ...
                let tool_use_block = ToolUseBlock::builder()
                    ...
                    .build()
                    .map_err(|e| BedrockError::MessageConversion(
                        format!("Failed to build tool use block: {}", e)
                    ))?;
...
fn determine_image_format(mime_type: &str) -> Result<aws_sdk_bedrockruntime::types::ImageFormat, BedrockError> {
    match mime_type {
        ...
        _ => Err(BedrockError::MessageConversion(
...
fn determine_document_format(mime_type: &str) -> Result<aws_sdk_bedrockruntime::types::DocumentFormat, BedrockError> {
    match mime_type {
        ...
        _ => Err(BedrockError::MessageConversion(
            format!("Unsupported document format: {}", mime_type)
        )),
    }
}
...
fn convert_content_blocks_to_parts(content_blocks: &[ContentBlock]) -> Result<Vec<Part>, BedrockError> {
        ...
        let part = match block {
            ...
            ContentBlock::Image(image_block) => {
                ...
                let mime_type = match format {
                    ...
                    _ => return Err(BedrockError::MessageConversion("Unknown image format".to_string())),
                };
            ...
            ContentBlock::ToolResult(tool_result) => {
                let content_text = match tool_result.content() {
                    [aws_sdk_bedrockruntime::types::ToolResultContentBlock::Text(text)] => text.clone(),
                    content_blocks => {
                        content_blocks.iter()
                            .filter_map(|block| match block {
                                aws_sdk_bedrockruntime::types::ToolResultContentBlock::Text(text) => Some(text.as_str()),
                                _ => None,
                            })
                            .collect::<Vec<_>>()
                            .join("")
...
pub(super) fn convert_token_usage_to_ai_ox(token_usage: aws_sdk_bedrockruntime::types::TokenUsage) -> Usage {
...
pub(super) fn convert_ai_ox_tools_to_bedrock(tools: Vec<Tool>) -> Result<Vec<aws_sdk_bedrockruntime::types::Tool>, BedrockError> {
            ...
            Tool::FunctionDeclarations(functions) => {
                for func in functions {
                    ...
                    let tool_spec = ToolSpecification::builder()
                        ...
                        .build()
                        .map_err(|e| BedrockError::MessageConversion(
                            format!("Failed to build tool specification: {}", e)
                        ))?;
                    tool_specs.push(aws_sdk_bedrockruntime::types::Tool::ToolSpec(tool_spec));
                }
            },
            #[cfg(feature = "gemini")]
            Tool::GeminiTool(_) => return Err(BedrockError::MessageConversion(
...
mod tests {
    use super::*;
    use crate::{
        content::{message::Message, part::Part, message::MessageRole},
        tool::{Tool, FunctionMetadata},
    };
    use aws_sdk_bedrockruntime::types::*;
    use serde_json::json;
    use chrono::Utc;
    ...
    fn test_determine_image_formats() {
        assert!(matches!(determine_image_format("image/png").unwrap(), ImageFormat::Png));
        assert!(matches!(determine_image_format("image/jpeg").unwrap(), ImageFormat::Jpeg));
        assert!(matches!(determine_image_format("image/jpg").unwrap(), ImageFormat::Jpeg));
        assert!(matches!(determine_image_format("image/gif").unwrap(), ImageFormat::Gif));
        assert!(matches!(determine_image_format("image/webp").unwrap(), ImageFormat::Webp));
    ...
    fn test_determine_document_formats() {
        assert!(matches!(determine_document_format("application/pdf").unwrap(), DocumentFormat::Pdf));
        assert!(matches!(determine_document_format("text/csv").unwrap(), DocumentFormat::Csv));
        assert!(matches!(determine_document_format("text/html").unwrap(), DocumentFormat::Html));
        assert!(matches!(determine_document_format("text/plain").unwrap(), DocumentFormat::Txt));
        assert!(matches!(determine_document_format("text/markdown").unwrap(), DocumentFormat::Md));
    ...
    fn test_convert_ai_ox_tools_to_bedrock() {
        ...
        match &result[0] {
            aws_sdk_bedrockruntime::types::Tool::ToolSpec(tool_spec) => {
                ...
                assert_eq!(tool_spec.description(), Some("Get weather information").as_deref());
            },
    ...
    fn test_convert_gemini_tool_error() {
        ...
        assert!(matches!(result.unwrap_err(), BedrockError::MessageConversion(_)));
    }

    #[test]
    fn test_convert_bedrock_finish_reasons() {
        assert!(matches!(convert_bedrock_finish_reason(StopReason::EndTurn), FinishReason::Stop));
        assert!(matches!(convert_bedrock_finish_reason(StopReason::ToolUse), FinishReason::ToolCalls));
        assert!(matches!(convert_bedrock_finish_reason(StopReason::MaxTokens), FinishReason::Length));
        assert!(matches!(convert_bedrock_finish_reason(StopReason::StopSequence), FinishReason::Stop));
        assert!(matches!(convert_bedrock_finish_reason(StopReason::ContentFiltered), FinishReason::ContentFilter));
        assert!(matches!(convert_bedrock_finish_reason(StopReason::GuardrailIntervened), FinishReason::ContentFilter));
    }
    ...
    fn test_convert_token_usage() {
        ...
        assert_eq!(result.input_tokens_by_modality.get(&crate::usage::Modality::Text), Some(&100));
        assert_eq!(result.output_tokens_by_modality.get(&crate::usage::Modality::Text), Some(&50));
    }
