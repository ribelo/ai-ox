use mistral_ox::{
    message::{
        AssistantMessage as MistralAssistantMessage, AudioContent as MistralAudioContent,
        ContentPart as MistralContentPart, Message as MistralMessage,
        SystemMessage as MistralSystemMessage, TextContent as MistralTextContent,
        ToolMessage as MistralToolMessage, UserMessage as MistralUserMessage,
    },
    request::ChatRequest,
    response::{ChatCompletionChunk, ChatResponse},
    tool::{
        Tool as MistralTool, ToolCall as MistralToolCall, ToolChoice as MistralToolChoice,
        ToolFunction,
    },
};
...
use crate::{
    ...
    model::ModelRequest,
    tool::{ToolUse, decode_tool_result_parts, encode_tool_result_parts},
    usage::Usage,
};
...
pub fn convert_request_to_mistral(
    ...
    if let Some(system_msg) = system_instruction {
        mistral_messages.push(MistralMessage::System(MistralSystemMessage::text(
            system_msg,
        )));
    }
...
fn convert_message_to_mistral(
    message: Message,
) -> Result<Vec<MistralMessage>, GenerateContentError> {
    match message.role {
        MessageRole::User => {
                ...
                match part {
                    ...
                    Part::Blob {
                        ...
                        if mime_type.starts_with("audio/") {
                            match data_ref {
                                DataRef::Uri { uri } => {
                                    text_parts.push(MistralContentPart::Audio(
                                        MistralAudioContent::new(uri),
                                    ));
                                }
                                DataRef::Base64 { .. } => {
                                    return Err(GenerateContentError::message_conversion(format!(
                                        "Mistral does not support base64-encoded audio. Upload the audio and provide a URI instead. (MIME type: {})",
                                        mime_type
                                    )));
                                }
                            }
                            ...
                            return Err(GenerateContentError::message_conversion(format!(
                                "Mistral does not support {} content in messages. Only audio content is supported.",
                                mime_type
                            )));
                        }
                    }
                    ...
                    Part::ToolResult {
                        ...
                        messages.push(MistralMessage::Tool(MistralToolMessage::new(
                            id,
                            content_str,
                        )));
                    }
                    Part::Opaque { provider, kind, .. } => {
                        return Err(GenerateContentError::message_conversion(format!(
                            "Cannot convert opaque content from provider '{}' of type '{}' to Mistral format",
                            provider, kind
                        )));
                    }
                }
            ...
            if messages.is_empty() {
                messages.push(MistralMessage::User(MistralUserMessage::new(Vec::<
                    MistralContentPart,
                >::new(
                ))));
            }
                ...
                match part {
                    ...
                    Part::Blob { mime_type, .. } => {
                        if mime_type.starts_with("image/") {
                            ...
                            return Err(GenerateContentError::message_conversion(format!(
                                "Mistral does not support {} content in assistant messages",
                                mime_type
                            )));
                        }
                    }
        ...
        MessageRole::System => {
            return Err(GenerateContentError::message_conversion(
                "System messages not supported in Mistral",
            ));
        }
...
fn convert_tools_to_mistral(
    tools: Vec<crate::tool::Tool>,
) -> Result<Vec<ai_ox_common::openai_format::Tool>, GenerateContentError> {
...
mod tests {
        ...
        let ai_msg = Message {
            ...
            content: vec![
                ...
                Part::ToolResult {
                    ...
                    parts: vec![Part::text(
                        serde_json::to_string(&json!({"temperature": 72, "condition": "sunny"}))
                            .unwrap(),
                    )],
                    ext: BTreeMap::new(),
                },
                Part::ToolResult {
                    ...
                    name: "get_news".into(),
                    parts: vec![Part::text(
                        serde_json::to_string(&json!({"headlines": ["News 1", "News 2"]})).unwrap(),
                    )],
                    ext: BTreeMap::new(),
                },
            ],
        ...
        let ai_msg = Message {
            ...
            content: vec![
                Part::ToolResult {
                    ...
                    name: "func1".into(),
                    parts: vec![Part::text(
                        serde_json::to_string(&json!({"result": "A"})).unwrap(),
                    )],
                    ext: BTreeMap::new(),
                },
                Part::ToolResult {
                    ...
                    name: "func2".into(),
                    parts: vec![Part::text(
                        serde_json::to_string(&json!({"result": "B"})).unwrap(),
                    )],
                    ext: BTreeMap::new(),
                },
            ],
        ...
        let ai_msg = Message {
            ...
            content: vec![
                ...
                Part::ToolResult {
                    ...
                    name: "func1".into(),
                    parts: vec![Part::text(
                        serde_json::to_string(&json!({"data": 1})).unwrap(),
                    )],
                    ext: BTreeMap::new(),
                },
                Part::Text {
                ...
                Part::ToolResult {
                    ...
                    name: "func2".into(),
                    parts: vec![Part::text(
                        serde_json::to_string(&json!({"data": 2})).unwrap(),
                    )],
                    ext: BTreeMap::new(),
                },
                Part::Text {
                    text: "Third text".into(),
                    ext: BTreeMap::new(),
                },
            ],
            timestamp: Some(Utc::now()),
    ...
    fn test_user_message_with_audio_content() {
        ...
        let ai_msg = Message {
            ...
            content: vec![
                Part::Text {
                    text: "What is being said in this audio?".into(),
                    ext: BTreeMap::new(),
                },
                Part::Blob {
    ...
    fn test_user_message_with_unsupported_image_returns_error() {
        ...
        assert!(
            error
                .to_string()
                .contains("Mistral does not currently support images")
        );
    }
    ...
    fn test_user_message_with_base64_audio_returns_error() {
        ...
        assert!(
            error
                .to_string()
                .contains("Mistral does not support base64-encoded audio")
        );
    ...
    fn test_assistant_message_with_unsupported_image_returns_error() {
        ...
        assert!(
            error
                .to_string()
                .contains("Mistral does not support images in assistant messages")
        );
    }
}
