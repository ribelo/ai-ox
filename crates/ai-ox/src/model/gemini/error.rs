...
pub enum GeminiError {
    ...
    #[error(
        "Missing API key. Please set it via the builder or the GOOGLE_AI_API_KEY environment variable."
    )]
    MissingApiKey,
        ...
        match error {
            GeminiError::Api(api_error) => match api_error {
                ...
                GeminiRequestError::JsonDeserializationError(json_err) => {
                    GenerateContentError::response_parsing(format!(
                        "Gemini JSON deserialization error: {}",
                        json_err
                    ))
                }
                ...
                GeminiRequestError::UrlBuildError(url_error) => {
                    GenerateContentError::configuration(format!(
                        "Gemini URL build error: {}",
                        url_error
                    ))
                }
                GeminiRequestError::IoError(io_error) => GenerateContentError::provider_error(
                    "gemini",
                    format!("IO error: {}", io_error),
                ),
                GeminiRequestError::AuthenticationMissing => GenerateContentError::configuration(
                    "Gemini authentication is missing: no API key or OAuth token provided",
                ),
            },
            ...
            GeminiError::Env(var_error) => GenerateContentError::configuration(format!(
                "Gemini environment variable error: {}",
                var_error
            )),
            ...
            GeminiError::MessageConversion(msg) => GenerateContentError::message_conversion(
                format!("Gemini message conversion: {}", msg),
            ),
        }
    }
}
