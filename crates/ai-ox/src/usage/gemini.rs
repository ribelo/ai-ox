...
use crate::usage::{Usage, Modality};
...
impl From<gemini_ox::generate_content::usage::Modality> for Modality {
    fn from(gemini_modality: gemini_ox::generate_content::usage::Modality) -> Self {
        match gemini_modality {
            gemini_ox::generate_content::usage::Modality::ModalityUnspecified => Modality::Other("unspecified".to_string()),
...
impl From<UsageMetadata> for Usage {
    fn from(gemini_usage: UsageMetadata) -> Self {
        ...
        if let Some(prompt_details) = &gemini_usage.prompt_tokens_details {
            details_map.insert("prompt_tokens_details".to_string(), serde_json::to_value(prompt_details).unwrap_or(Value::Null));
        }
        
        if let Some(cache_details) = &gemini_usage.cache_tokens_details {
            details_map.insert("cache_tokens_details".to_string(), serde_json::to_value(cache_details).unwrap_or(Value::Null));
        }
        
        if let Some(candidates_details) = &gemini_usage.candidates_tokens_details {
            details_map.insert("candidates_tokens_details".to_string(), serde_json::to_value(candidates_details).unwrap_or(Value::Null));
        }
        
        if let Some(tool_details) = &gemini_usage.tool_use_prompt_tokens_details {
            details_map.insert("tool_use_prompt_tokens_details".to_string(), serde_json::to_value(tool_details).unwrap_or(Value::Null));
        }
        ...
        Usage {
            ...
            details: if details_map.is_empty() { None } else { Some(Value::Object(details_map)) },
        }
    }
}
...
mod tests {
    ...
    fn test_modality_conversion() {
        ...
        assert_eq!(ai_ox_unspecified, Modality::Other("unspecified".to_string()));
    }
    ...
    fn test_usage_conversion_basic() {
        ...
        assert_eq!(*ai_ox_usage.input_tokens_by_modality.get(&Modality::Text).unwrap(), 100);
        assert_eq!(*ai_ox_usage.output_tokens_by_modality.get(&Modality::Text).unwrap(), 75);
        assert_eq!(*ai_ox_usage.cache_tokens_by_modality.get(&Modality::Text).unwrap(), 50);
    }
