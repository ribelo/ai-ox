...
impl Serialize for ApiRequestError {
        ...
        S: Serializer,
    {
        match self {
            ApiRequestError::ReqwestError(e) => {
                let mut state = serializer.serialize_struct("ApiRequestError", 2)?;
                state.serialize_field("type", "ReqwestError")?;
                state.serialize_field("error", &e.to_string())?;
                state.end()
            }
            ApiRequestError::SerdeError(e) => {
                let mut state = serializer.serialize_struct("ApiRequestError", 2)?;
                state.serialize_field("type", "SerdeError")?;
                state.serialize_field("error", &e.to_string())?;
                state.end()
            }
            ApiRequestError::JsonDeserializationError(e) => {
                let mut state = serializer.serialize_struct("ApiRequestError", 2)?;
            ...
            ApiRequestError::InvalidRequestError {
                ...
                let mut state = serializer.serialize_struct("ApiRequestError", field_count)?;
            ...
            ApiRequestError::InvalidEventData(message) => {
                let mut state = serializer.serialize_struct("ApiRequestError", 2)?;
            ...
            ApiRequestError::RateLimit => {
                let mut state = serializer.serialize_struct("ApiRequestError", 1)?;
                state.serialize_field("type", "RateLimit")?;
                state.end()
            }
            ApiRequestError::UrlBuildError(message) => {
                let mut state = serializer.serialize_struct("ApiRequestError", 2)?;
                state.serialize_field("type", "UrlBuildError")?;
                state.serialize_field("message", message)?;
                state.end()
            }
            ApiRequestError::IoError(e) => {
                let mut state = serializer.serialize_struct("ApiRequestError", 2)?;
                state.serialize_field("type", "IoError")?;
                state.serialize_field("error", &e.to_string())?;
                state.end()
            }
            ApiRequestError::Stream(message) => {
                let mut state = serializer.serialize_struct("ApiRequestError", 2)?;
...
pub fn parse_error_response(status: reqwest::StatusCode, bytes: Vec<u8>) -> ApiRequestError {
    ...
    if let Ok(payload) = serde_json::from_slice::<OpenRouterErrorPayload>(&bytes) {
        ...
        ApiRequestError::UnexpectedResponse(format!(
            "HTTP status {}: {}",
            status.as_u16(),
            error_text
        ))
