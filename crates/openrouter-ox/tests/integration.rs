...
mod tests {
    ...
    use openrouter_ox::tool::{Tool, ToolChoice, FunctionMetadata};
    use openrouter_ox::{OpenRouter, ChatRequest};
    ...
    async fn test_list_models() {
        ...
        assert!(model_ids.iter().any(|id| id.contains("gpt") || id.contains("claude") || id.contains("llama")));
    }
    ...
    async fn test_provider_preferences() {
        ...
        use openrouter_ox::provider_preference::{ProviderPreferences, Provider};
        request.provider = Some(ProviderPreferences {
            allow_fallbacks: None,
            require_parameters: None,
            data_collection: None,
            order: Some(vec![Provider::OpenAI, Provider::Anthropic, Provider::Google]),
            only: None,
            ignore: None,
            quantizations: None,
            sort: None,
...
fn get_test_client() -> Result<openrouter_ox::OpenRouter, Box<dyn std::error::Error>> {
    use openrouter_ox::{OpenRouter, ChatRequest};
    use openrouter_ox::message::Message;
    OpenRouter::load_from_env().map_err(|e| format!("Failed to load OpenRouter API key: {}. Set OPENROUTER_API_KEY environment variable.", e).into())
}
