...
impl GenerateContentRequest {
    ...
    ) -> BoxStream<'static, Result<GenerateContentResponse, GeminiRequestError>>
    {
        ...
        let api_key = gemini.api_key.clone();
        let project_id = gemini.project_id.clone();
        let api_version = gemini.api_version.clone();
        let base_url = gemini.base_url().to_string();
        ...
        Box::pin(try_stream! {
            ...
            if is_oauth {
                ...
                url.path_segments_mut()
                    .map_err(|_| GeminiRequestError::UrlBuildError("URL cannot be a base URL".to_string()))?
                    .push("v1internal:streamGenerateContent");
            ...
            let request_body = if is_oauth {
                ...
                let minimal_request = serde_json::json!({
                    ...
                    "systemInstruction": request_data.system_instruction
                });
                serde_json::json!({
                    "model": request_data.model.to_string(),
                    "project": project_id,
                    "request": minimal_request
                })
            } else {
            ...
            match status.as_u16() {
                 200 | 201 => {
                     ...
                     // Buffer to accumulate bytes and process complete lines
                     let mut line_buffer = String::new();

                     while let Some(chunk_result) = byte_stream.next().await {
                         let bytes = chunk_result?; // Propagate reqwest stream errors

                         // Append chunk to buffer, handling potential UTF-8 errors within the chunk
                         match std::str::from_utf8(&bytes) {
                            Ok(s) => line_buffer.push_str(s),
                            Err(e) => {
                                // Handle invalid UTF-8 in the stream by yielding an error
                                Err(GeminiRequestError::InvalidEventData(format!("Invalid UTF-8 sequence in stream chunk: {e}")))?;
                            }
                         }

                         // Process all complete lines currently in the buffer
                         while let Some(newline_pos) = line_buffer.find('\n') {
                             // Drain the line (including \n) from the buffer
                             let line_part = line_buffer.drain(..=newline_pos).collect::<String>();
                             // Trim whitespace and the newline character (handles \r\n implicitely via trim)
                             let line = line_part.trim();

                             if line.is_empty() {
                                 continue; // Skip empty lines (e.g., between messages)
                             }
                             ...
                             if let Some(json_data) = line.strip_prefix("data:") {
                                 ...
                                 // Ignore SSE comments (often used for keep-alive)
                             } else {
                                 // Ignore other SSE lines that don't match expected formats
                             }
                         }
                         // Any data left in line_buffer is an incomplete line; wait for the next chunk.
                     }
                 ...
                 429 => Err(GeminiRequestError::RateLimit)?,
                 _ => {
                     // Handle error response body by reading it once and parsing it
                     let body_bytes = res.bytes().await?;
