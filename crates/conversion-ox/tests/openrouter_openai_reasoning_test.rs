...
fn test_openrouter_openai_reasoning_conversion() {
    ...
    if let AnthropicContent::Thinking(thinking) = &anthropic_response.content[0] {
        ...
        assert!(thinking.text.contains("I need to tackle this classic riddle step by step"));
        assert!(thinking.text.contains("Let's craft that final output"));
    } else {
        panic!("Expected thinking content from OpenRouter→OpenAI reasoning, got: {:?}", anthropic_response.content[0]);
    }
...
fn test_anthropic_thinking_to_openrouter_openai_request() {
        ...
        .messages(vec![
            AnthropicMessage {
                ...
                content: StringOrContents::Contents(vec![AnthropicContent::Text(
                    anthropic_ox::message::Text::new("A farmer has 17 sheep. All but 9 die. How many sheep are left?".to_string())
                )]),
            },
            AnthropicMessage {
                ...
                content: StringOrContents::Contents(vec![
                    ...
                    AnthropicContent::Text(
                        anthropic_ox::message::Text::new("Let me work through this step by step...".to_string())
                    ),
                ]),
            },
        ])
    ...
    if let OpenRouterMessage::Assistant(assistant_msg) = &openrouter_request.messages[1] {
        ...
        if let ContentPart::Text(text_content) = &assistant_msg.content[0] {
            assert!(text_content.text.contains("I need to solve this sheep riddle carefully"));
        ...
        if let ContentPart::Text(text_content) = &assistant_msg.content[1] {
            assert_eq!(text_content.text, "Let me work through this step by step...");
        } else {
