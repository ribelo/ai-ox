            ...
            AnthropicMessage {
                ...
                content: StringOrContents::Contents(vec![
                    ...
                    AnthropicContent::Text(anthropic_ox::message::Text::new(
                        "Let me calculate 15 * 23 = 345".to_string(),
                    )),
                ]),
            },
        ])
...
fn test_openrouter_to_anthropic_thinking_conversion() {
    ...
    if let AnthropicContent::Thinking(thinking) = &anthropic_response.content[0] {
        assert_eq!(
            thinking.text,
            "Let me work through this step by step. 15 × 23 means I need to multiply these two numbers. I can break this down: 15 × 20 = 300, and 15 × 3 = 45. So 300 + 45 = 345."
        );
    } else {
        panic!(
            "Expected thinking content, got: {:?}",
...
fn test_anthropic_to_openrouter_to_anthropic_round_trip() {
    ...
    let original_thinking = ThinkingContent::new(
        "This is a complex reasoning process that I need to work through carefully.".to_string(),
    );
        ...
        .messages(vec![
            ...
            AnthropicMessage {
                ...
                content: StringOrContents::Contents(vec![
                    AnthropicContent::Thinking(thinking_with_signature),
                    AnthropicContent::Text(anthropic_ox::message::Text::new(
                        "Here is my solution to the problem.".to_string(),
                    )),
                ]),
            },
        ])
    ...
    let reasoning_detail = ReasoningDetail {
        detail_type: "reasoning.text".to_string(),
        text: Some(
            "This is a complex reasoning process that I need to work through carefully."
                .to_string(),
        ),
        summary: None,
        data: None,
...
fn test_openrouter_to_anthropic_to_openrouter_round_trip() {
    ...
    let original_reasoning_detail = ReasoningDetail {
        detail_type: "reasoning.text".to_string(),
        text: Some(
            "I need to analyze this request carefully and provide a thoughtful response."
                .to_string(),
        ),
        summary: None,
        data: None,
...
fn test_full_round_trip_thinking_preservation() {
    ...
    if let AnthropicContent::Thinking(thinking) = &final_response.content[0] {
        assert_eq!(
            thinking.text,
            "Let me think about this problem systematically. First, I need to understand what is being asked."
        );
        // Note: signature is lost in OpenRouter round-trip (expected limitation)
    } else {
        panic!(
            "Expected thinking content, got: {:?}",
            final_response.content[0]
        );
    }
